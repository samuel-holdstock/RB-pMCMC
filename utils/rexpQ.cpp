...
unsigned int get_m(double rho, double prec, unsigned int mlo) {
  if (rho > 4.2e9) {
    return 0;
  }
...
bool commonChecks(double prec, int vn_rows, int vn_cols, int Qn_rows, int Qn_cols, string sinit, bool ignore_v) {
  bool bad=false;
...
bool sChecks(const arma::mat v, const arma::sp_mat Q, double prec, string caller, bool ignore_v) {
...
bool dChecks(const arma::mat v, const arma::dmat Q, double prec, string caller, bool ignore_v) {
  string sinit="ERROR ("+caller+"): ";
  bool bad=commonChecks(prec, v.n_rows, v.n_cols, Q.n_rows, Q.n_cols, sinit, ignore_v);
