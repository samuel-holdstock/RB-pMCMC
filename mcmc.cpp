...
const arma::vec &tout_list, const int &iterations, const int &num_particles, const arma::mat &sigma){
  ...
  double acceptance=0;
  double scale = 1*sqrt(2.38*2.38/num_params);
  for(int i=0;i<iterations;++i){
    ...
    if(log(unif[i])<alpha){
      ...
      // std::cout<<"Iteration:"<<i<<", ThetaPost:"<<theta_posterior<<", PsiProposal"<<psi_posterior<<", Accepted:"<<exp(alpha)<<std::endl;
      ...
      // std::cout<<"Iteration:"<<i<<", ThetaPost:"<<theta_posterior<<", PsiProposal"<<psi_posterior<<", Rejected:"<<exp(alpha)<<std::endl;
...
//[[Rcpp::export]]
Rcpp::List mcmc_RB2(const std::string &model_name, const arma::vec &start, const arma::vec &thetas, 
const arma::mat &obs_list, const arma::mat &lower_list, const arma::mat &upper_list, 
const arma::vec &tout_list, const int &iterations, const int &num_particles, const arma::mat &sigma){
  int num_params = thetas.size();
  arma::mat posterior(iterations,num_params);
  ...
  for(int i=0;i<iterations;++i){
    transformed_psi = transformed_theta + offsets.col(i)*scale;
    // std::cout<<"Iteration:"<<i<<", Acceptance:"<<acceptance/(i+1)<<", Proposal"<<transformed_psi.t()<<std::endl;;
    psi_ll = get_likelihood_frac(model_name,start,inv_transform(transformed_theta),obs_list,tout_list,num_particles);
    psi_posterior = get_prior(transformed_psi)+psi_ll;
    // alpha = psi_ll-theta_ll;
    alpha = psi_posterior-theta_posterior;    
    if(isnan(alpha)){
...
const arma::mat &obs_list, const arma::vec &tout_list, const int &iterations, const int &num_particles, const arma::mat &sigma){
  int num_params = thetas.size();
  arma::mat posterior(iterations,num_params);
  arma::vec ll(iterations);
  arma::vec transformed_theta = transform(thetas);
...
double get_prior(const arma::vec &transformed_theta){
  ...
  // arma::vec mus = {0.5,0.0025,0.3};
  // arma::vec transformed_mus = {-0.43275, -5.99021, -1.050226}; // transform(mus)
  arma::vec transformed_mus = {-0.6931, -5.9914, -1.2040}; // transform(mus)
  arma::vec transformed_sigmas = {0.5,0.5,0.5};
  double log_prior = 0;
